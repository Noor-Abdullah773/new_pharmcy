...
/*class DrugList extends StatefulWidget {
  ...
  const DrugList({super.key, required this.c});
                ...
                builder: (ctx, d, child) {
                  return Container(
                    ...
                    child: Badge(
                      ...
                      label: Text("${DonationVM.cart.length}"),
                    ),
                  );
                },
              ),
        ...
        children: [
          ...
          Expanded(
            child: FutureBuilder<List<Donation>>(
              future: dvm.getData(ApiUrls.approvedCategory),
              builder: (ctx, snapshot) {
                if (snapshot.connectionState == ConnectionState.done) {
                  List<Donation>? specificCategory = snapshot.data
                      ?.where((element) => element.category_id == widget.c.id)
                      .toList();

                  if (specificCategory != null && specificCategory.isNotEmpty) {
                        ...
                        return Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const DottedLine(),
                              const SizedBox(height: 5),
                              if (category.type == "ادوية")
                                Row(
                                  mainAxisAlignment:
                                      MainAxisAlignment.spaceAround,
                                  children: [
                                    const Text('شكل الدواء',
                                        style: TextStyle(fontFamily: 'myfont')),
                                    Text('${category.shape}'),
                                  ],
                                ),
                    ...
                    return Center(child: Text("لا توجد تبرعات"));
                  }
                } else {
                  return Center(child: CircularProgressIndicator());
                }
